...
function convertToRomanNumerals(num) {
  ...
  let i = 0;
  while (i < romanMap.length) {
    while (value >= romanMap[i].val) {
      result += romanMap[i].str;
      value -= romanMap[i].val;
    }
    i += 1;
  }
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < size; i += 1) {
    for (let j = 0; j < Math.floor(size / 2); j += 1) {
      const temp = matrixCopy[i][j];
      matrixCopy[i][j] = matrixCopy[i][size - 1 - j];
      matrixCopy[i][size - 1 - j] = temp;
    }
  }
...
function shuffleChar(str, iterations) {
  ...
  for (let i = 0; i < iterations; i += 1) {
    if (seen.has(currentString)) {
      const cycleLength = i - seen.get(currentString);
